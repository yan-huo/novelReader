    ...
    >
      <view :style="{ transform: translateY, transition: transition }">
        <view
          ...
          :class="[
            { 'is-success': isShowDownTip && isDownSuccess },
            { 'is-error': isShowDownTip && isDownError }
          ]"
        ...
        <slot name="up-error" v-else-if="isUpError && showUpError">
          <view class="s-pull-tip-wrap" v-if="upErrorText" @click="onUpErrorClick">{{
            upErrorText
          }}</view>
        </slot>
  ...
  methods: {
    ...
    touchmove(e) {
      ...
      if (
        moveY > 0 &&
        (scrollRealTop <= 0 ||
          (scrollRealTop <= this.numDownStartTop && scrollRealTop === this.startTop))
      ) {
        // 可下拉的条件
        if (
          this.pullDown &&
